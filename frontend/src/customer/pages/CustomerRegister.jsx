...
function CustomerRegister({ onRegistered, onBackToHome }) {
  ...
  const requestOtp = async (event) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/v1/auth/otp/register`,
        {
          method: "POST",
          headers: {
  ...
  const verifyOtp = async (event) => {
    ...
    try {
      ...
      onRegistered(data.accessToken, mobileNumber);
    } catch (error) {
      setMessage(error.message);
      setMessageType("error");
    } finally {
